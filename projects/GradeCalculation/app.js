...
time_line
  ...
  .fromTo(animation, 0.3, { opacity: 1 }, { opacity: 0 }); //動畫跑一秒後消失

window.setTimeout(() => {
  animation.style.pointerEvents = "none";
}, 2500); // 因為原本動畫跑完消失其實還是在 所以要設定他在消失後就可以點選後面的背景
...
let allSelects = document.querySelectorAll("select"); // 這會給我們一個靜態的nodelist
...
// 改變 credit之後，GPA也要更新
let changeCredit = document.querySelectorAll(".class-credit");
changeCredit.forEach((credit) => {
...
function changeColor(target) {
  if (target.value == "A" || target.value == "A-") {
    target.style = "background-color: lightgreen  ;color:black";
  } else if (
    target.value == "B" ||
    target.value == "B-" ||
    target.value == "B+"
  ) {
    target.style = "background-color: yellow;color:black";
  } else if (
    target.value == "C" ||
    target.value == "C-" ||
    target.value == "C+"
    ...
    target.style = "background-color: grey;color:black";
  } else {
    target.style = "background-color: grey;color:black";
  }
}
...
function setGPA() {
  let formLength = document.querySelectorAll("form").length; //這是一個nodelist 所以可以使用length來看長度
...
allTrash.forEach((trash) => {
  trash.addEventListener("click", (e) => {
    // e.target 就是指 trash本身
    e.target.parentElement.parentElement.classList.add("remove"); //先建立動畫
    let form = trash.parentElement.parentElement; // 這是指到 form的位置
    form.addEventListener("transitionend", (e) => {
      //transitionend 表示當tansition結束後要執行這個callback function
      e.target.remove();
      setGPA();
    });
  });
